...
class Trainer():
    def __init__(self,args):
        ...
        self.model,self.proj_model= self.get_model()
    ...
    def get_optimizer(self):
        ...
        optimizer_grouped_parameters = [
                ...
                {
                    "params": [p for n, p in self.proj_model.named_parameters() if not any(nd in n for nd in no_decay) and p.requires_grad],
                    "weight_decay": self.args.weight_decay,
                    'lr': self.args.lr,
                },
                # 这个组包含了bias和LayerNorm的所有参数，不应用权重衰减
                {
                    "params":[p for n, p in self.proj_model.named_parameters() if any(nd in n for nd in no_decay) and p.requires_grad],
                    "weight_decay": 0.0,
                    'lr': self.args.lr,
                }
...
#         model.model.embed_tokens.weight.data[-self.args.num_nodes:]=self.data.x
    ...
    def get_prompt(self,batch,is_training=True):
        ...
        for i,label in zip(neighbors_dict.keys(),batch.y[:batch.batch_size]):
            ...
            if is_training :
                text = template['train'].format('node_'+str(i),connect_text,'node_'+str(i),label)+'</s>'
            else:
                text = template['test'].format('node_'+str(i),connect_text,'node_'+str(i))
    ...
    def train(self):
        ...
        self.proj_model=self.proj_model.cuda()
        ...
        for epoch in range(self.args.epoch):
            ...
            for step_i, batch in enumerate(self.train_loader):
                ...
                if step_i % self.args.gradient_accumulation_steps == 0:
                    ...
                    parameters = list(self.proj_model.parameters())+list(self.model.parameters())
                    ...
                    for param in self.model.parameters():
                        param.grad = None
                    for param in self.proj_model.parameters():
                        param.grad = None
        ...
        torch.save(self.proj_model.state_dict(),"proj_{}_end.pth".format(self.args.epoch))
                                         
    def test(self):
        for epoch in range(1):
            ckpt_path = "llmcom_2_end.pth"
            proj_path = "proj_2_end.pth"
            self.model.load_state_dict(torch.load(ckpt_path), strict=True)  
            self.proj_model.load_state_dict(torch.load(proj_path), strict=True)  
